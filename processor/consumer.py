...
def main():
    print("📥 Iniciando Motor de Antifraude Real-Time... Aguardando dados do Kafka...")
    
    grupo_teste = f'motor-antifraude-{uuid.uuid4()}'
    
    consumer = KafkaConsumer(
        'transacoes_pendentes',
        bootstrap_servers=['localhost:9092'], 
        api_version=(3, 7, 0), # 1. Versão alinhada com o Producer e Broker
        auto_offset_reset='earliest', 
        enable_auto_commit=False, # 2. Desativado para cumprir o Requisito 13
        group_id='motor-antifraude-oficial', # 3. Grupo estático para evitar rebalanceamentos infinitos
        value_deserializer=lambda v: json.loads(v.decode('utf-8'))
    )

    print("✅ Negociação de Grupo concluída. Lendo transações...")

    try:
        for mensagem in consumer:
            transacao = mensagem.value
            transacao['offset'] = mensagem.offset
            # Processa a fraude
            processar_transacao(transacao)
            # 4. Commit manual APÓS o processamento (At-least-once)
            consumer.commit()

    except KeyboardInterrupt:
        print("\n🛑 Motor Antifraude encerrado manualmente.")
    finally:
        consumer.close()
